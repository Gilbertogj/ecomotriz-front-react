...
export const FormAgregarReporteOperador = ({ reporte }) => {
  ...
  const siguienteEtapa = useMemo(() => {
    ...
    if (!reporte.inicio_bombeo) return "inicio_bombeo";
    if (!reporte.fin_bombeo) return "fin_bombeo";
  ...
  const handleSelectsSubmit = async (e) => {
    try {
      ...
      const url = `${process.env.REACT_APP_API_CONCRECO_BACKEND_URL}/api_produccion/reporte_operador/${id}/update/`;
  ...
  return (
    <div>
      <ModalRedirect
        ...
        link="/concreco/produccion/operador"
      />
      <ModalRedirect
        showConfirmModal={showModal}
        text="Hora marcada correctamente."
        link={`/concreco/produccion/pedido/${reporte.pedido}`}
      />
      ...
      {siguienteEtapa === "inicio_bombeo" &&
        (!isLoading ? (
          <form
            onSubmit={(e) => {
              registrarHora(e, "inicio_bombeo");
            }}
          >
            <input
              type="submit"
      ...
      {siguienteEtapa === "fin_bombeo" &&
        (!isLoading ? (
          <form
            onSubmit={(e) => {
              registrarHora(e, "fin_bombeo");
            }}
          >
            <input
              type="submit"
